...
totales = {'temp': [], 'temp_min': [],
           'temp_max': []}  # lista de mediciones de todos los días donde almacenamos los valores obtenidos.
for medicion in datos['list']:
    ...
    if not dias.get(dia):
        dias[dia] = {'temp': [], 'temp_min': [],
                     'temp_max': []}  # en este caso, tiene un diccionario con tres listas vacías, diferencia respeto v2
...
if directorio == "":
    ...
    for dia, temps in dias.items():  # recorremos los pares (dia para recorrer las claves, temps para recorrer los valores) items (recorre los pares)
        ...
        print(
            f'Día {dia[8:]}-{dia[5:7]}-{dia[0:4]}: \tTemperatura media: {(sum(temps["temp"]) / len(temps["temp"])):.2f}º\t'
    ...
    try:
        file_name = ciudad + "_" + (datos["list"][0]["dt_txt"][:10]) + "_" + (datos["list"][0]["dt_txt"][11:]) + "_" + (
                    datos["list"][39]["dt_txt"][:10] + ".html")
